...
func MakeAppState(ctx, serverShutdownCtx context.Context, options *swag.CommandLineOptionsGroup) *state.State {
	...
	enterrors.GoWrapper(func() {
		...
		// executor construction time) can run the audit using the
		// same lookup. The wrapper inside RestoreClassDir is the
		// authoritative trigger for the post-restore case: it fires
		// AFTER the file move from `.backup.tmp/<class>/` to the
		// final `<class>/<shard>/lsm/...` layout, which is the
		// moment orphan tracker dirs become visible on disk.
		//
		// 0-weaviate-issues#215 B3 Gap 1 — see the
		// restoreClassDirWithAudit wrapper above for the full
		// rationale.
		repo.SetReindexAuditDeps(knownTask, appState.Logger)
	}, appState.Logger)
